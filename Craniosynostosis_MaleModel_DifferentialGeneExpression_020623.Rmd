---
title: "Craniosynostosis RNA sequencing: Male- Differential Gene expression"
...
## Male Craniosynostosis RNAseq: Differential Gene Expression
...
```{r Load Data}
load("/active/paquette_a/slapehn/CranioRNAseq/Male_CranioRNAseq/Data/Male_CranioRNAseq_VoomAdjustedData.RData")
```

## Save Expression Data for Supp Table
Save expression data for supplemental table 1 construction
```{r Expr Data}
genes <- v$genes
logcpm_male <- v$E
...
Combine all DEGs into one dataset for comparison to other models
```{r All DEGs}
...
Core_VOI_DEGs <- inner_join(All_Male_DEGs, Core_VOIs, by=c("hgnc_symbol"="VOIs.Core_Variants"))

#Step 3: Compare DEGs with Occasional VOIs
Occ_VOIs <- data.frame(VOIs$Occasional_Variants)
Occ_VOI_DEGs <- inner_join(All_Male_DEGs, Occ_VOIs, by=c("hgnc_symbol"="VOIs.Occasional_Variants"))

```

##Editable Plot 
Save editable copies of volcano plots to ensure that DEG names do not overlap
```{r editable plot}
#Load Packages
library(tidyr)
...
doc <- read_pptx()
#ppt file_name
ppt<-paste0("Male Model Volcano Plots", format(Sys.time(), "%d_%b_%Y"), ".pptx")
ppt
