---
title: "Female Craniosynostosis RNAseq:  Differential Gene Expression"
...
## Female Craniosynostosis RNAseq: Differential Gene Expression

This code performs the differential expression analysis for the Cunningham Lab Craniosynostosis RNAseq data Female Samples.
...
## Load Data
Load filtered and voom adjusted data
```{r Load Data}
load("/active/paquette_a/slapehn/CranioRNAseq/Female_CranioRNAseq/Data/Female_CranioRNAseq_VoomAdjustedData.RData")
```


## Save Expression Data for Supp Table
Save expression data for construction of Supplemental Table 1
```{r Expr Data}
genes <- v$genes
logcpm_female <- v$E
logcpm_female_DF <-data.frame(logcpm_female)
logcpm_female_DF$mean_logcpm <- rowMeans(logcpm_female_DF)
logcpm_female_DF$ensembl <- rownames(logcpm_female_DF)

logcpm_female_all <- full_join(logcpm_female_DF, genes, by=c("ensembl"="ensembl_gene_id")) %>%
...
```{r Volcano}
MakeVPlot_Down<-function(Results,Title){
...
plot(Results$logFC,-log(Results$P.Value),ylim=c(0,25),xlim=c(-3.5, 3.5), pch=20,cex=0.35,main=Title,ylab="Negative Log p-value",xlab="Log Fold Change",col="grey60")
points(sig_down$logFC,-log(sig_down$P.Value),pch=20,cex=1,col=ColorPalette[4])
text(sig_down_top$logFC,-log(sig_down_top$P.Value),sig_down_top$hgnc_symbol,cex=1,col="Black",pos=1)
}

MakeVPlot_Down(AllResults_Coronal,"Coronal")
MakeVPlot_Down(AllResults_Lambdoid,"Lambdoid")
MakeVPlot_Down(AllResults_Sagittal,"Sagittal")
MakeVPlot_Down(AllResults_Metopic, "Metopic")
```
...
bardf %>% ggplot(aes(x=Phenotype, y=Count, fill=Direction)) + 
  ...
  ggtitle("Female-DEG Direction by Craniosynostosis Phenotype") + 
...
Upset_list<-list("Coronal"=Cor_Vec, "Lambdoid"=Lamb_Vec, "Sagittal"=Sag_Vec, "Metopic"=Met_Vec)
...
```
#Create dataframe of all DEGs across phenotypes
```{r all degs}
...
All_Female_DEGs <- full_join(Sig_Cor_Lamb, Sig_Met_Sag, by=c("hgnc_symbol"))
```
...
Core_VOI_DEGs <- inner_join(All_Female_DEGs, Core_VOIs, by=c("hgnc_symbol"="VOIs.Core_Variants"))

#Step 3: Compare DEGs with Occasional VOIs
Occ_VOIs <- data.frame(VOIs$Occasional_Variants)
Occ_VOI_DEGs <- inner_join(All_Female_DEGs, Occ_VOIs, by=c("hgnc_symbol"="VOIs.Occasional_Variants"))

```


##Editable Plot 
Save volcano plots as editable in case there is overlap of gene names
```{r editable plot}
#Load Packages
library(tidyr)
...
doc <- read_pptx()
#ppt file_name
ppt<-paste0("Female Model Volcano Plots", format(Sys.time(), "%d_%b_%Y"), ".pptx")
ppt
...
VPlot_Coronal_Editable <- rvg::dml(code = MakeVPlot_Down(AllResults_Coronal,"Coronal"))
VPlot_Metopic_Editable <- rvg::dml(code = MakeVPlot_Down(AllResults_Metopic,"Metopic"))
VPlot_Sagittal_Editable <- rvg::dml(code = MakeVPlot_Down(AllResults_Sagittal,"Sagittal"))
VPlot_Lambdoid_Editable <- rvg::dml(code = MakeVPlot_Down(AllResults_Lambdoid,"Lambdoid"))
#add slide
