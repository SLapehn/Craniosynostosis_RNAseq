---
title: "Craniosynostosis RNAseq- Male Model Filtering"
...
## Male Craniosynostosis RNAseq- Data Filtering
...
```{r covariate load}
CranioCovariates <- read.csv("/Volumes/paquette_a/slapehn/CranioRNAseq/Male_CranioRNAseq/Male_Cranio_CovariateData.csv")
...
Loading count data with rownames as entrez gene IDs. Removing three participants that have missing covariate data and subsetting to only include Male participants.
```{r count data load}
...
Counts$"163630" <- NULL
Counts$"95689" <- NULL
Male_SampleIDs <- CranioCovariates$Sample_ID
Male_Counts <- Counts[colnames(Counts) %in% Male_SampleIDs]
Count_Matrix <- data.matrix(Male_Counts)

```

##Create an annotation file using BioMart
...
```{r Entrez to Ensembl}
Male_Counts$Entrez <- rownames(Male_Counts)
annot$Entrez <- rownames(annot)
Entrez_Ensembl_Map <- annot %>%
  dplyr::select(ensembl_gene_id, Entrez)

Counts_Ensembl <- inner_join(Entrez_Ensembl_Map, Male_Counts, by=c("Entrez"="Entrez"))
annot$Entrez <- NULL
...
#2.  Remove all genes that are not protein coding
...
```{r save}
save(CranioCovariates, y_Final, file="/Volumes/paquette_a/slapehn/CranioRNAseq/Male_CranioRNAseq/Data/Male_CranioRNAseq_FilteredNormalized.RData")
```
