---
title: "Craniosynostosis RNAseq- Combined model Differential Gene Expression"
...
## Craniosynostosis RNAseq:Combined Model Differential Gene Expression 

This code performs the differential expression analysis with Limma for the single suture Craniosynostosis RNAseq data from the. 
## Load Packages
```{r Packages}
library(tidyverse)
library(limma)
...
## Load Data
Load Data from SVA and Voom performed in "Craniosynostosis_CombinedModel_SVAandVoom.Rmd"
```{r Load Data}
load("/Volumes/paquette_a/slapehn/CranioRNAseq/CranioRNAseq_VoomAdjustedData.RData")
```

## Save Expression Data for Supp Table
Pulling expression data to save for supplemental table
```{r Expr Data}
genes <- v$genes
logcpm_primary <- v$E
...
write.csv(logcpm_primary_all, "/Volumes/paquette_a/slapehn/CranioRNAseq/DEGs/logcpm_primary.csv")
```

## Run Full Model on voom transformed data
Run differential gene expression model using the Limma pipeline
...
##DEG Volcano Plots
Visualize DEG significance as a volcano plot for each phenotype
```{r Volcano}
...
Visualize DEG number and directionality across phenotypes as a bar graph
```{r Plot DEGs}
...
DEGNumbers_Plot <- c(38, 34, 18, 15, 48, 55, 46, 43)
...
write.csv(Sig_AllPhenotypes, "/Volumes/paquette_a/slapehn/CranioRNAseq/Figures/Heatmap/SigAllPhenotypes.csv")
Sig_AllPhenotypes_Sorted <- read.csv("/Volumes/paquette_a/slapehn/CranioRNAseq/Figures/Heatmap/SigAllPhenotypes_Sorted.csv")
...
Create a dataframe of all DEGS from all models to use for comparison against Male and Female models
```{r combine}
...
write.csv(All_Combined_DEGs, "/Volumes/paquette_a/slapehn/CranioRNAseq/DEGs/All_Combined_DEGs.csv")
